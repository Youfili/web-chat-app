...
func (rt *_router) liveness(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if err := rt.db.Ping(); err != nil {
		...
		_, _ = w.Write([]byte(`{"status": "error", "message": "database not reachable"}`))
		return
	}

	// 2. Se il Ping ha successo, rispondo con 200 OK
	w.WriteHeader(http.StatusOK)

	// Scrivo un piccolo body così "curl" mi mostra qualcosa
	_, _ = w.Write([]byte(`{"status": "up", "service": "Wasatext"}`))
}
